...
func (h *HpClientHandler) ChannelActive(session *smux.Session) {
	h.Session = session
	h.Active = true
...
func (h *HpClientHandler) ChannelRead(stream *smux.Stream, data interface{}) {
...
func (h *HpClientHandler) ChannelInactive(stream *smux.Stream) {
	if stream != nil {
		stream.Close()
	} else {
		h.Session.Close()
...
func (h *HpClientHandler) connected(stream *smux.Stream, message *hpMessage.HpMessage) {
...
// writeData 往内网写数据
func (h *HpClientHandler) WriteData(stream *smux.Stream, message *hpMessage.HpMessage) {
...
// writeOutData 往外网写数据
func (h *HpClientHandler) writeOutData(stream *smux.Stream, message []byte) error {
	if h.OutLimit != nil {
		b := h.OutLimit.Burst()
		for {
			end := len(message)
