...
func (h *CmdClientHandler) ChannelActive(conn net.Conn) {
	log.Infof("CMD指令激活 ip:%s", conn.RemoteAddr().String())
}
...
func (h *CmdClientHandler) ChannelRead(conn net.Conn, data interface{}) error {
	defer func() {
		if err := recover(); err != nil {
			// 捕获异常并记录日志
			log.Errorf("CMD-ChannelRead: %v\n栈情况: %s", err, string(debug.Stack()))
		}
	}()
	message := data.(*cmdMessage.CmdMessage)
	if message == nil {
		log.Infof("CMD消息类型:解码异常|ip:%s", conn.RemoteAddr().String())
		return errors.New("消息类型异常")
	}
	if message.Type != cmdMessage.CmdMessage_TIPS {
		log.Infof("消息类型:%s|消息版本:%s|ip:%s", message.Type.String(), message.Version, conn.RemoteAddr().String())
	}
	switch message.Type {
...
func (h *CmdClientHandler) ChannelInactive(conn net.Conn) {
	log.Infof("CMD指令断开 ip:%s", conn.RemoteAddr().String())
	h.Clear(conn)
}
